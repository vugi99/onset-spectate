...
function speclogic(cmdply,ply)
    for i,v in ipairs(spectable) do
     ...
     tblinsert.y = y
     tblinsert.z = z
     if GetPlayerDimension(cmdply) ~= GetPlayerDimension(ply) then
        tblinsert.dim = GetPlayerDimension(cmdply)
        SetPlayerDimension(cmdply, GetPlayerDimension(ply))
     end
...
AddRemoteEvent("NoLongerSpectating",function(ply)
    for i,v in ipairs(spectable) do
        if v.id == ply then
           ...
           if posbefspec[i].dim then
              SetPlayerDimension(ply,posbefspec[i].dim)
           end
           table.remove(posbefspec,i)
           SetPlayerPropertyValue(ply, "Spectating", false, true)
        end
     end
end)
